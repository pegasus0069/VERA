...
static void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    if (lv_disp_flush_is_last(drv)) {
        ...
        esp_lcd_panel_draw_bitmap(panel_handle, 0, 0, LVGL_PORT_H_RES, LVGL_PORT_V_RES, color_map);
...
static lv_disp_t *display_init(esp_lcd_panel_handle_t panel_handle)
{
    ...
    esp_err_t err = esp_lcd_rgb_panel_get_frame_buffer(panel_handle, 3, &lvgl_port_rgb_last_buf, &buf1, &buf2);
    ...
    void *fbs[3];
    esp_err_t err = esp_lcd_rgb_panel_get_frame_buffer(panel_handle, 3, &fbs[0], &fbs[1], &fbs[2]);
    if (err != ESP_OK) return NULL;
    buf1 = fbs[2]; // Set buf1 to the third frame buffer
#else
    esp_err_t err = esp_lcd_rgb_panel_get_frame_buffer(panel_handle, 2, &buf1, &buf2); // Get two frame buffers
    if (err != ESP_OK) return NULL;
#endif
#else
...
esp_err_t esp_lv_adapter_init(esp_lcd_panel_handle_t lcd_handle, esp_lcd_touch_handle_t tp_handle)
{
    ...
    if (tp_handle) {
        ...
        if (!indev) {
            ESP_LOGW(TAG, "Failed to initialize touch input device");
        }
...
IRAM_ATTR bool esp_lv_adapter_notify_rgb_vsync(void)
{
    ...
    if (lvgl_task_handle != NULL) {
        vTaskNotifyGiveFromISR(lvgl_task_handle, &need_yield); // Notify the LVGL task
    }
#endif
