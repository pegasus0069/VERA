...
esp_lcd_panel_handle_t waveshare_esp32_s3_rgb_lcd_init()
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "esp_lcd_new_rgb_panel failed: 0x%x (PSRAM not enabled or insufficient memory)", ret);
        return NULL;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "esp_lcd_panel_init failed: 0x%x", ret);
        return NULL;
    }

    // Clear framebuffers to black immediately to avoid uninitialized PSRAM artifacts / scanline strikes on startup
    void *fb1 = NULL;
    void *fb2 = NULL;
    if (esp_lcd_rgb_panel_get_frame_buffer(panel_handle, 2, &fb1, &fb2) == ESP_OK) {
        if (fb1) memset(fb1, 0, EXAMPLE_LCD_H_RES * EXAMPLE_LCD_V_RES * sizeof(uint16_t));
        if (fb2) memset(fb2, 0, EXAMPLE_LCD_H_RES * EXAMPLE_LCD_V_RES * sizeof(uint16_t));
    }
