...
void IO_EXTENSION_Pwm_Output(uint8_t Value)
{
    ...
    // Hardware PWM on CH32V003 is ACTIVE-LOW:
    // 0 = 100% duty (Maximum Brightness)
    // 255 = 0% duty (Display Off)
    // Value = 100% -> hw_duty = 0 (Maximum Brightness)
    // Value = 10%  -> hw_duty = 218 (Dim night mode)
    // Value = 0%   -> hw_duty = 255 (Completely Off)
    uint8_t hw_duty;
    if (Value == 0) {
        hw_duty = 255;
    } else {
        hw_duty = (uint8_t)((100 - Value) * 240 / 99);
    }

    uint8_t data[2] = {IO_EXTENSION_PWM_ADDR, hw_duty};
    // Write the 8-bit duty value to the CH32V003 PWM register
    DEV_I2C_Write_Nbyte(IO_EXTENSION.addr, data, 2);
}
